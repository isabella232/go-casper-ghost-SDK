...
// Simple Serialize specification.
func ForkRoot(fork *core.Fork) ([32]byte, error) {
...
// Simple Serialize specification.
func CheckpointRoot(hasher HashFn, checkpoint *core.Checkpoint) ([32]byte, error) {
...
func HistoricalRootsRoot(historicalRoots [][]byte) ([32]byte, error) {
	result, err := BitwiseMerkleize(hashutil.CustomSHA256Hasher(), historicalRoots, uint64(len(historicalRoots)), params.ChainConfig.HistoricalRootsLimit)
	if err != nil {
...
func SlashingsRoot(slashings []uint64) ([32]byte, error) {
	slashingMarshaling := make([][]byte, params.ChainConfig.EpochsPerSlashingVector)
